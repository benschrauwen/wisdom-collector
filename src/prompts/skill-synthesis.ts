...
export function buildSkillSynthesisSystemPrompt(book: BookMetadata): string {
  return `
You are turning book notes into a reusable skill document or small skill family.

Your output is not a book report. It is the actual skill content another agent should read and apply directly.
...
- Borrow the spirit of strong skill writing: keep the frontmatter focused on triggering, keep the body lean, and use only as much structure as the material actually needs.
- Treat \`skillBody\` as the real markdown body of \`SKILL.md\`. The structured fields are only transport rails for the pipeline; do not write the body like a rigid form if a different layout would teach the skill better.
